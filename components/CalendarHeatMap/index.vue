    ...
    <svg class="vch__wrapper" :viewBox="viewBox">
        <g class="vch__months__labels__wrapper" :transform="monthsLabelWrapperTransform[position]">
            <text class="vch__month__label"
                  ...
                  :x="getMonthLabelPosition(month).x" :y="getMonthLabelPosition(month).y">{{ lo.months[month.value] }}
            </text>
        </g>
        <g class="vch__days__labels__wrapper" :transform="daysLabelWrapperTransform[position]">
            <text class="vch__day__label" :x="vertical ? SQUARE_SIZE * 1 : 0"
                  :y="vertical ? SQUARE_SIZE - SQUARE_BORDER_SIZE : 20">{{ lo.days[1] }}
            </text>
            <text class="vch__day__label" :x="vertical ? SQUARE_SIZE * 3 : 0"
                  :y="vertical ? SQUARE_SIZE - SQUARE_BORDER_SIZE : 44">{{ lo.days[3] }}
            </text>
            <text class="vch__day__label" :x="vertical ? SQUARE_SIZE * 5 : 0"
                  :y="vertical ? SQUARE_SIZE - SQUARE_BORDER_SIZE : 69">{{ lo.days[5] }}
            </text>
        </g>
        <g class="vch__legend__wrapper" :transform="legendWrapperTransform[position]">
            ...
            <rect v-for="(color, index) in rangeColor" :key="index" :style="{ fill: color }"
                  :width="SQUARE_SIZE - SQUARE_BORDER_SIZE" :height="SQUARE_SIZE - SQUARE_BORDER_SIZE"
                  :x="vertical ? SQUARE_SIZE * 1.75 : SQUARE_SIZE * index"
                  :y="vertical ? SQUARE_SIZE * (index + 1) : 5"></rect>
            <text :x="vertical ? SQUARE_SIZE * 1.25 : SQUARE_SIZE * rangeColor.length + 1"
        ...
        <g class="vch__year__wrapper" :transform="yearWrapperTransform">
            <g class="vch__month__wrapper" v-for="(week, weekIndex) in heatMap.calendar" :key="weekIndex"
               :transform="getWeekPosition(weekIndex)">
                <rect v-for="(day, dayIndex) in week" class="vch__day__square" v-if="day.date < now"
                      :key="dayIndex" :transform="getDayPosition(dayIndex)"
                      :width="SQUARE_SIZE - SQUARE_BORDER_SIZE" :height="SQUARE_SIZE - SQUARE_BORDER_SIZE"
                      :style="{ fill: rangeColor[day.colorIndex] }" @click="$emit('day-click', day)"></rect>
            </g>
        </g>
    </svg>
</template>
